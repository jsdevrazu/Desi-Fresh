...
const BannerCard = ({ path, title, leftTitle, smText, price, color }) => {
  return (
    <>
      <div className="flex items-center gap-2 bg-white shadow-lg w-[45%] ml-4 mb-4">
        <div className="px-4 flex justify-center items-center flex-col">
          <img src={path} alt="Desi" />
          <h2 className="text-primary text-sm font-semibold text-center">{title}</h2>
        </div>
        <div
          className="px-2 py-2 rounded-md flex-col flex justify-center items-center text-center h-full"
